...
kafka_yc - это кластер из 3-х Apache Nifi сервисов, координируемых сервисом Zookeeper. Apache Nifi  включает два процессора: один читает данных из топика в Managed Apache Kafka и передает эти данные второму процессору, который преобразует данные из Avro и складывает в таблицу в Postgres.

За генерацию и отправку данных в Managed Apache Kafka отвечает сервис *app_producer*. Помимо получения данных из Postgres (куда данные отправляются с помощью Apache Nifi), можно так же запустить *app_consumer* (тянет сообщения напрямую из брокеров).
...
!!!!!!!!!! AvroReaderTest и AvroReaderDatabase это один и тот же процессор, мне просто пришлось пересоздавать и я забыл переименовать. Пишу если возникнет вопрос, почему на скринах AvroReaderTest, а описываю AvroReaderDatabase !!!!!!!!!!!!!
